...
def execute_voice_command(text):
    ...
    # Common Google transcription variants. These are intentionally limited
    # to commands already supported by AccessAI.
    replacements = {
        "open google chrome": "open chrome",
        "launch google chrome": "launch chrome",
        "start google chrome": "start chrome",
        "open calculator app": "open calculator",
        "go backwards": "go back",
        "go backward": "go back",
        "reload page": "refresh",
        "scroll downward": "scroll down",
        "scroll upwards": "scroll up",
        "scroll upward": "scroll up",
        "left click": "click",
        "click it": "click",
    }
    command = replacements.get(command, command)
...
def voice_loop():
    ...
    Voice path:
        Windows/WASAPI -> WAV -> SpeechRecognition AudioFile -> Google
    """
    global voice_status
    ...
    if not SOUNDCARD_AVAILABLE:
        ...
        print("   Run: python -m pip install soundcard numpy")
        return
    ...
    print("   Pipeline: Windows/WASAPI → WAV → Google SpeechRecognition")
    print()
    ...
    while not voice_stop_event.is_set() and running:
        ...
        try:
            ...
            wav_path = _record_wav_with_soundcard()
            ...
            # Try Indian English first, then US English using the exact
            # same captured audio. Both are supported by Google's recognizer.
            try:
                ...
                print("   ⚠ en-IN could not understand; retrying as en-US...")
                text = recognizer.recognize_google(
                    audio,
                    language="en-US"
                )
