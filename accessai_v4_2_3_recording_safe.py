...
#   Enter / Space
#   Stop AccessAI
#   Typing mode / keyboard input
#
# V4.1 FIX
# Typing commands are recognized before the normal intent classifier,
# with tolerant matching for common Whisper variations.
#
# IMPORTANT FIX
...
# Prevent facial tilt scrolling from firing at the same time as a voice command.
VOICE_SCROLL_SUPPRESS_SECONDS = 2.5
...
VOICE_INITIAL_PROMPT = (
    ...
    "AccessAI typing mode, typing mode, exit typing mode, exit typing, stop typing, command mode, return to command mode, start typing, press enter, press backspace, select all, copy, paste, stop AccessAI. Natural dictated text may be ordinary sentences."
)
...
def _clean_typed_text(text):
    """Prepare Whisper text for natural direct keyboard input."""
    text = (text or "").strip()

    # Whisper often adds sentence punctuation even when the user did not
    # say punctuation. Remove it so typing mode does not add a full stop
    # after every spoken sentence.
    text = re.sub(r"[.!?]+(?=\s|$)", "", text)

    # Remove other automatic punctuation while keeping apostrophes inside
    ...
    # Normalize whitespace around the resulting text without destroying
    # intentional newlines/tabs.
    text = re.sub(r"[ \t]+", " ", text)
    text = re.sub(r" *\n *", "\n", text)
    return text.strip()
...
def _handle_text_input(text):
    ...
    # Helpful diagnostic: this lets us see exactly what Whisper delivered
    # and whether typing mode was already active.
    print(f"[Typing handler] heard={command!r} | mode={typing_mode}")
    ...
    # Whisper can produce small variations such as "typing",
    # "typing mood", or "start typing mode". Handle those locally
    # instead of sending them through the normal intent classifier.
    enter_typing_phrases = {
        "typing", "typing mode", "start typing", "start typing mode",
        "enable typing", "enable typing mode", "enter typing",
        "enter typing mode", "begin typing", "begin typing mode",
        "activate typing", "activate typing mode"
    }
    exit_typing_phrases = {
        "exit typing", "exit typing mode", "stop typing",
        "stop typing mode", "disable typing", "disable typing mode",
        "end typing", "end typing mode", "leave typing",
        "leave typing mode"
    }

    def _close_match(value, choices, threshold=0.78):
        if value in choices:
            return True
        return any(
            difflib.SequenceMatcher(None, value, candidate).ratio() >= threshold
            for candidate in choices
        )

    # Only allow the ENTER command while we are NOT already in typing mode.
    # Previously, fuzzy matching could interpret "stop typing" as "start typing",
    # causing typing mode to turn ON again before the exit handler was reached.
    if not typing_mode and _close_match(command, enter_typing_phrases, 0.76):
        typing_mode = True
    ...
    # ------------------------------------------------------------
    # ROBUST TYPING-MODE EXIT
    # ------------------------------------------------------------
    # Whisper can mishear "exit typing mode" as things such as:
    # "exit typing", "exit typing mood", "exiting typing mode",
    # "exciting typing mode", "stop typing mode", etc.
    # Treat these as exit commands BEFORE ordinary speech is typed.
    if typing_mode:
        exit_starters = {
            "exit", "exiting", "exciting", "exited",
            "stop", "stopping",
            "disable", "disabled",
            "end", "ending",
            "leave", "leaving",
        ...
        # Very common direct forms.
        direct_exit_forms = {
            "exit typing",
            "exit typing mode",
            "exit typing mood",
            "exiting typing mode",
            "exciting typing mode",
            "stop typing",
            "stop typing mode",
            "stop typing mood",
            "disable typing",
        ...
        if command in direct_exit_forms:
            typing_mode = False
            set_status("COMMAND MODE", 2)
            print("⌨ COMMAND MODE — typing stopped")
            return True

        # Fuzzy two-word detection. This catches a Whisper transcript
        # whose first word is slightly wrong but whose intent is clear,
        # e.g. "exciting typing mode".
        if len(words) <= 5 and "typing" in words:
            first = words[0]
            if (
                first in exit_starters
                or any(
                    difflib.SequenceMatcher(None, first, candidate).ratio() >= 0.62
                    for candidate in exit_starters
                )
            ):
                typing_mode = False
                set_status("COMMAND MODE", 2)
                print(f"⌨ COMMAND MODE — typing stopped (heard: {command!r})")
                return True

        # Also accept "typing mood" / "typing mode" with a preceding
        # exit-like word that Whisper may have slightly distorted.
        if len(words) <= 5 and len(words) >= 2 and words[1] in {"typing", "typingmood", "typingmode"}:
            first = words[0]
            if any(
                difflib.SequenceMatcher(None, first, candidate).ratio() >= 0.62
                for candidate in exit_starters
    ...
    if _close_match(command, exit_typing_phrases, 0.82):
        typing_mode = False
        set_status("COMMAND MODE", 2)
        print("⌨ COMMAND MODE — typing stopped")
        return True
    ...
    if typing_mode:
        ...
        if typed:
            pyautogui.write(typed, interval=0.005)
            # Add a space only when the user actually continues typing;
            # do not manufacture punctuation or extra characters.
            pyautogui.write(" ", interval=0.005)
...
def execute_voice_command(text):
    ...
    global running, last_voice_command_time, tilt_state, tilt_candidate, tilt_candidate_frames, tilt_release_frames

    # A recognized voice command may happen while the user is still tilting
    # their head. Suppress facial scrolling briefly so voice actions such as
    # refresh/back do not accidentally cause an additional scroll.
    last_voice_command_time = time.time()
    # Clear any in-progress facial tilt so a voice command cannot inherit
    # the previous head position and trigger scrolling during recording.
    tilt_state = 0
    tilt_candidate = 0
    tilt_candidate_frames = 0
    tilt_release_frames = 0
...
def _record_audio_with_soundcard():
    """Capture voice, using a longer window while typing naturally."""
    ...
    # Commands stay fast, but typing mode gets more time so longer spoken
    # sentences are not cut off after a couple of words.
    if typing_mode:
        max_record_seconds = 5.0
        silence_pause = 0.70
    else:
        max_record_seconds = VOICE_RECORD_SECONDS
        silence_pause = VOICE_IDLE_PAUSE

    max_frames = int(max_record_seconds * VOICE_SAMPLE_RATE)
    pre_roll_chunks = 4
    silence_chunks_needed = max(1, int(silence_pause / 0.10))
    min_speech_chunks = 2
...
def _transcribe_with_whisper(audio):
    ...
    active_beam = 3 if typing_mode else VOICE_BEAM_SIZE
    active_best_of = 3 if typing_mode else 1
            ...
            if should_process:
                ...
                if result is not None and result.face_landmarks:
                    ...
                    if (
                        ...
                        >= SCROLL_INTERVAL
                        and now - last_voice_command_time
                        >= VOICE_SCROLL_SUPPRESS_SECONDS
                    ):
